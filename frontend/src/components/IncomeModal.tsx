...
import CarPaymentModal from './CarPaymentModal';
...
const IncomeModal: React.FC<IncomeModalProps> = ({ isOpen, onClose }) => {
  ...
  const { data: debtsResponse, isLoading: debtsLoading } = useDebts({ type: 'receivable' });
  const debts = Array.isArray(debtsResponse?.debts) 
    ? debtsResponse.debts.filter((debt: any) => {
