                      ...
                      <div className="flex-1">
                        {apprentice.profession && (
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-sm font-medium text-blue-700">{t('Kasbi', language)}:</span>
                            <span className="text-sm font-semibold text-blue-900">{apprentice.profession}</span>
                          </div>
                        )}
                        {apprentice.experience !== undefined && apprentice.experience > 0 && (
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-blue-700">{t('Tajriba', language)}:</span>
                            <span className="text-sm font-semibold text-blue-900">{apprentice.experience} {t('yil', language)}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {apprentice.email && (
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-gray-500 mb-0.5">{t('Email', language)}</div>
                        <div className="text-sm font-medium text-gray-900 truncate">{apprentice.email}</div>
                      </div>
                    </div>
                  )}
                  {apprentice.phone && (
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-gray-500 mb-0.5">{t('Telefon raqam', language)}</div>
                        <div className="text-sm font-medium text-gray-900">{formatPhoneNumber(apprentice.phone)}</div>
                      </div>
                    </div>
                  )}
                  <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-gray-500 mb-0.5">{t("Qo'shilgan sana", language)}</div>
                      <div className="text-sm font-medium text-gray-900">{new Date(apprentice.createdAt).toLocaleDateString('uz-UZ')}</div>
                    </div>
                  </div>
                </div>
                ...
                {apprentice.percentage !== undefined && (
                  <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-4 border-2 border-purple-200">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-sm text-purple-600 mb-1">{t('Foiz ulushi', language)}</div>
                        <div className="text-3xl font-bold text-purple-900">{apprentice.percentage}%</div>
                      </div>
                      <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-purple-500 text-white text-2xl font-bold shadow-lg">
                        %
                      </div>
                    </div>
                    <p className="text-xs text-purple-600 mt-2">
                      {t("Vazifa to'lovidan oladigan ulushi", language)}
                    </p>
                  </div>
                )}
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border-2 border-green-200">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-green-900">{t('Daromad', language)}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white rounded-lg p-3 border border-green-200">
                      <div className="text-xs text-green-600 mb-1">{t('Joriy oylik', language)}</div>
                      <div className="text-lg font-bold text-green-900">{formatCurrency(apprentice.earnings || 0)}</div>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-green-200">
                      <div className="text-xs text-green-600 mb-1">{t('Jami daromad', language)}</div>
                      <div className="text-lg font-bold text-green-900">{formatCurrency(apprentice.totalEarnings || 0)}</div>
                    </div>
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <Award className="h-4 w-4 text-purple-600 flex-shrink-0" />
                    <span className="text-sm font-semibold text-purple-600">{stats.awards} {t('mukofot', language)}</span>
                  </div>
                </div>
